...
public class Applicant implements Comparable<Applicant> {
	...
	private void parseGrades(String gradesAsString) {
		...
		for (int i = 0; i < g.length; i++) {
			if (g[i].equals("U") || g[i].equals("A") || g[i].equals("B")) {
				// Om underkänd så räknar vi det som en nolla
				grades[i] = 0;
				
      		}
			
			else if(Integer.parseInt(g[i]) < 3 || Integer.parseInt(g[i]) > 5){
				grades[i] = 0;
			}			
			else {
	...
	public String toString() {
		String grade = Integer.toString(grades[0]);
		for(int i = 1; i<grades.length; i++) {
			grade = grade + "," + grades[i];
		}
		return name + " " + "[" + grade + "]"+ "(avg: "+getAvgGrade()+ ")" ;
	} 
